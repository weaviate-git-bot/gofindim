...
func InsertIntoWeaviate(img *ImageFile) error {
	...
	keypoints, descriptors := GetImageSiftDescriptors(img)
	keypointsBlob, err := keypointsToBytes(keypoints)
	if err != nil {
		log.Fatal(err)
		panic(err)
	}
	// descriptorsBlob := descriptors.ToBytes()

	encodedKPs := base64.StdEncoding.EncodeToString(keypointsBlob)
	descriptorsVectors := matToFloat32Array(descriptors)
	if 1 == 2 {
		println("descriptorsVectors: ", descriptorsVectors)
		println("descriptorsVectors: ", encodedKPs)
	}

	imageInterface := map[string]interface{}{
		"keypoints":  encodedKPs, //keypointsBlob,
		"descriptor": descriptorsVectors,
		"imageData": []map[string]string{
			{"beacon": "weaviate://localhost/" + dataUid.String()},
		},
	}
